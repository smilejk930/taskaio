# taskaio Docker 빌드 스크립트 (Windows PowerShell)

# 한글 깨짐 방지를 위한 코드페이지 및 인코딩 설정
...
if (!(Get-Command pnpm -ErrorAction SilentlyContinue)) {
    Write-Error "Error: pnpm이 설치되어 있지 않습니다. pnpm을 설치해주세요."
    exit 1
}

# Docker 명령어 존재 여부 확인
if (!(Get-Command docker -ErrorAction SilentlyContinue)) {
    Write-Error "Error: Docker가 설치되어 있지 않거나 PATH에 추가되지 않았습니다."
    exit 1
}

# Docker 데몬 실행 여부 확인
Write-Host "Docker 데몬 상태 확인 중..." -ForegroundColor Cyan
docker version >$null 2>&1
if ($LASTEXITCODE -ne 0) {
    Write-Error "Error: Docker 데몬이 실행 중이지 않습니다. Docker Desktop을 실행해주세요."
    exit 1
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Error "Error: Next.js 빌드에 실패했습니다."
    exit 1
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Error "Error: Docker 이미지 빌드에 실패했습니다."
    exit 1
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Error "Error: 이미지 저장에 실패했습니다."
    exit 1
}

Write-Host "`n모든 프로세스가 완료되었습니다!" -ForegroundColor Green
