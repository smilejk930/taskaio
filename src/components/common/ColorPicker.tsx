...
export function ColorPicker({ 
    ...
    disabled, 
    title = '색상 지정',
    triggerClassName = 'w-6 h-6 rounded-full border border-border shadow-sm shrink-0'
}: ColorPickerProps) {
    const [isOpen, setIsOpen] = useState(false)
    const [tempColor, setTempColor] = useState(color || '#94a3b8')
    const colorInputRef = useRef<HTMLInputElement>(null)

    // 외부에서 color prop이 변경되면 tempColor 동기화 (예: 담당자 변경으로 인한 색상 자동 연동)
    useEffect(() => {
        setTempColor(color || '#94a3b8')
    }, [color])
    ...
    return (
        <Popover open={isOpen} onOpenChange={setIsOpen}>
            <PopoverTrigger asChild>
                <button
                    disabled={disabled}
                    className={`${triggerClassName} outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50 disabled:cursor-not-allowed`}
                    style={{ backgroundColor: color || '#94a3b8' }}
                    title={disabled ? "권한 없음" : title}
                />
            </PopoverTrigger>
