...
- **CLI API 필터**: 복수 상태·우선순위, 업무 기간, 관리 업무, 마감 임박·지연 조회와 업무 요약 API 제공
...
PAT 인증 기반 업무 API는 기존 단일 필터와 함께 다음 조회 조건을 지원합니다.
...
복수 상태·우선순위는 쉼표로 구분하며 같은 필터 내에서는 OR, 서로 다른 필터 간에는 AND로 처리합니다. `from`/`to`는 업무 시작일과 종료일이 모두 존재하고 조회 범위와 겹치는 업무를 반환합니다. `due=soon`은 `asOf`부터 3일 이내, `due=overdue`는 `asOf` 이전의 미완료 업무를 뜻합니다. `asOf`를 생략하면 서버 날짜를 사용합니다.

요약 API는 필터 결과를 기준으로 전체 업무와 1Depth 관리 업무의 상태별 수, 관리 업무 평균 진행률, 마감 임박·지연·마감일 없음 수를 반환합니다. 웹 화면의 기존 필터와 대시보드 동작은 변경하지 않습니다.
