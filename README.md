...
본 프로젝트는 에이전트 지향 개발 방식을 따릅니다. `.agent` 디렉토리에 정의된 규칙과 스킬을 바탕으로 에이전트에게 자연어로 지시하여 기능을 확장할 수 있습니다.

### 에이전트 컨텍스트 구조
- `.agent/rules/`: 프로젝트 코딩 컨벤션 및 보안 정책
- `.agent/skills/`: 프레임워크 및 라이브러리별 전문 지식 (Next.js, Supabase, Gantt 등)
- `.agent/workflows/`: 반복적인 작업(마이그레이션, 리뷰 등)을 위한 표준화된 절차
